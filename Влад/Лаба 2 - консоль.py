...
class Car():
    def __init__(self, number, brand, model, engine_type, color, power, colored):
        self.number = number
        self.brand = brand
        self.model = model
    ...
    def change(self):
        ...
        match parameter.split():
            ...
            case ["Крашена?"]:
                inp = input('Введите параметр покраски авто (да/нет): ')
                self.colored = inp
                S[first+6] = self.colored
...
while True:
    ...
    match option.split():
        case ["1"]:#вывести таблицу
            table = PrettyTable(["Номер", "Марка", "Модель", "Тип двигателя", "Цвет", "Мощность", "Крашена?"])
        ...
        case ["2"]:#добавить авто
            ...
            colored = input('Введите параметр покраски авто (да/нет): ')
            S_add = [num,' ', brand,' ', model,' ', engine_type,' ', color,' ', power,' ', colored,' ']
        ...
        case ["5"]:#поиск авто
            with open ("cars_table.txt", "r") as t:
                S = t.read().split() 
            tablesearch = PrettyTable(["Номер", "Марка", "Модель", "Тип двигателя", "Цвет", "Мощность", "Крашена?"])
